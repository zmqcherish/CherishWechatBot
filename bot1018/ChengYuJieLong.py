...
class ChengYuJieLong(ProcessInterface):
    ...
    def process(self, msg, type):
        ...
        cy = list(ChengyuColl.find({'cy': content}))
        ...
        if len(cy) == 0:
            ...
            return
        if cy[0]['first'] != ChengyuGroup[group_name]:
            # itchat.send('错误', groupId)
            return
        answers = list(ChengyuColl.find({'first': cy[0]['last']}))
        if len(answers) == 0:
            itchat.send('我不会', group_id)
        else:
            answer = random.choice(answers)
            ChengyuGroup[group_name] = answer['last']
